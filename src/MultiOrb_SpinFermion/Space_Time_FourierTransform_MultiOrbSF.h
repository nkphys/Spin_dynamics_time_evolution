...
#ifndef ST_Fourier_MultiOrbSF_
#define ST_Fourier_MultiOrbSF_

class ST_Fourier_MultiOrbSF{
...
*/
    ...
    SC_SW_ENGINE_VNE_MultiOrbSF& SC_SW_ENGINE_VNE_MultiOrbSF_;
...
void ST_Fourier_MultiOrbSF::Initialize_engine(){
...
void ST_Fourier_MultiOrbSF::Perform_Averaging_on_one_point(){
...
void ST_Fourier_MultiOrbSF::Perform_Smarter_Averaging_on_one_point(){
...
void ST_Fourier_MultiOrbSF::Calculate_SpaceTimeDisplacedCorrelations(string STdisplaced_Crt_fileout){
...
void ST_Fourier_MultiOrbSF::Calculate_SpaceTimeDisplacedCorrelations_Smarter(string STdisplaced_Crt_fileout){
    ...
    for(int conf=0;conf<No_Of_Inputs;conf++){
        ...
        for(int r=0;r<Parameters_.ns;r++){
            rx=Coordinates_.indx_cellwise(r);
            ry=Coordinates_.indy_cellwise(r);

            for(int rp=0;rp<rp_max;rp++){
                rpx=Coordinates_.indx_cellwise(rp);
                rpy=Coordinates_.indy_cellwise(rp);

                rx_new = (rpx+rx)%Parameters_.lx;
                ry_new = (rpy+ry)%Parameters_.ly;
                r_new = Coordinates_.Ncell_(rx_new, ry_new);
...
void ST_Fourier_MultiOrbSF::Calculate_Skw_from_Crt(string fileout){
...
void ST_Fourier_MultiOrbSF::Calculate_Skw_from_Crt_(string fileout){
...
void ST_Fourier_MultiOrbSF::Calculate_Sqw_using_Fwq(string fileout, string Dqw_file){
...
void ST_Fourier_MultiOrbSF::Calculate_Sqw_using_Aq_Fwq(string fileout, string Dqw_file){
...
void ST_Fourier_MultiOrbSF::Calculate_Skw_from_Srt_file( string filename, string fileout){
...
void ST_Fourier_MultiOrbSF::Read_parameters(){


    w_min=SC_SW_ENGINE_VNE_MultiOrbSF_.w_min;
    w_max=SC_SW_ENGINE_VNE_MultiOrbSF_.w_max;

    dw=SC_SW_ENGINE_VNE_MultiOrbSF_.dw;
    w_conv=SC_SW_ENGINE_VNE_MultiOrbSF_.w_conv;
    Use_FFT=SC_SW_ENGINE_VNE_MultiOrbSF_.Use_FFT;

    no_of_processors=SC_SW_ENGINE_VNE_MultiOrbSF_.no_of_processors;


}

void ST_Fourier_MultiOrbSF::Convolute_the_spectrum(string Sqw_file_in, string Sqw_file_out){
...
void ST_Fourier_MultiOrbSF::Sum_Rule_For_Way1(string Sqw_file_in, string Sq_static_out, string Sq_dynamical_out){
    ...
    for(int nx_=0;nx_<Parameters_.lx;nx_++){
        for(int ny_=0;ny_<Parameters_.ly;ny_++){
            k_ind = Coordinates_.Ncell_(nx_,ny_);
    ...
    for(int nx_=0;nx_<Parameters_.lx;nx_++){
        for(int ny_=0;ny_<Parameters_.ly;ny_++){
            k_index=Coordinates_.Ncell_(nx_,ny_);
