...
void Observables::Calculate_Skw(double mu){
...
void Observables::Get_red_den_mat(Mat_6_Complex_doub &Red_Den_mat, double mu){

    double Beta=Parameters_.beta;
    //Red_Den_mat; //c^dagger[j][orb][spin]c[l][orb2][spin2]
    int c1, c2;
    for (int j=0;j<Parameters_.ns;j++){
        for (int l=0;l<Parameters_.ns;l++){
            for (int o=0;o<Parameters_.orbs;o++){
                for (int o2=0;o2<Parameters_.orbs;o2++){
                    for (int s=0;s<2;s++){
                        for (int s2=0;s2<2;s2++){
                            for (int n=0;n<2;n++){
                                c1 = j + o*ns_+ s*ns_*Parameters_.orbs;
                                c2 = l + o2*ns_+ s2*ns_*Parameters_.orbs;
                                //probably conjugate
                                Red_Den_mat[j][o][s][l][o2][s2] =  conj(Hamiltonian_.Ham_(c1,n))*Hamiltonian_.Ham_(c2,n)*(1.0/(1.0 + exp(Beta*(Hamiltonian_.eigs_[n] - mu))));
