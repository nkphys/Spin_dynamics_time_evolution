...
void SC_SW_ENGINE_VNE_3orbPnictides::Initialize_engine(){
    ...
    for (int i=0;i<Parameters_.ns;i++){
        quant_s_x_eq[i].resize(Parameters_.orbs);
        quant_s_y_eq[i].resize(Parameters_.orbs);
        quant_s_z_eq[i].resize(Parameters_.orbs);
        quant_s_x[0][i].resize(Parameters_.orbs);
        quant_s_y[0][i].resize(Parameters_.orbs);
        quant_s_z[0][i].resize(Parameters_.orbs);
    }


    for(int i=0;i<Parameters_.ns;i++){
        Red_Den_mat[i].resize(Parameters_.orbs);
        for(int orb=0;orb<Parameters_.orbs;orb++){
            Red_Den_mat[i][orb].resize(2);
            for(int s=0;s<2;s++){
                Red_Den_mat[i][orb][s].resize(Parameters_.ns);
                for(int j=0;j<Parameters_.ns;j++){
                    Red_Den_mat[i][orb][s][j].resize(Parameters_.orbs);
    ...
    for(int pos=0;pos<Parameters_.ns;pos++){
        Jval_array[pos]=Parameters_.J_HUND;
        Bval_array[pos]=0.0;
    }
...
void SC_SW_ENGINE_VNE_3orbPnictides::Start_Engine(){
#ifdef _OPENMP
    ...
    for(int ts=0;ts<=time_steps;ts++){
        ...
        if(ts==0){
            ...
            for(int pos=0;pos<Parameters_.ns;pos++){
                for(int orb=0;orb<Parameters_.orbs;orb++){
                    quant_s_x_eq[pos][orb] = 0.5*real( Red_Den_mat[pos][orb][1][pos][orb][0] +
                            Red_Den_mat[pos][orb][0][pos][orb][1] );
                    quant_s_y_eq[pos][orb] = 0.5*imag( Red_Den_mat[pos][orb][1][pos][orb][0] -
                            Red_Den_mat[pos][orb][0][pos][orb][1] );
                    quant_s_z_eq[pos][orb] = 0.5*real( Red_Den_mat[pos][orb][0][pos][orb][0] -
                            Red_Den_mat[pos][orb][1][pos][orb][1] );
                }
            }
            ...
            for(int pos=0;pos<Parameters_.ns;pos++){
                for(int orb=0;orb<Parameters_.orbs;orb++){
                    quant_s_x[0][pos][orb] = 0.5*real( Red_Den_mat[pos][orb][1][pos][orb][0] +
                            Red_Den_mat[pos][orb][0][pos][orb][1] );
                    quant_s_y[0][pos][orb] = 0.5*imag( Red_Den_mat[pos][orb][1][pos][orb][0] -
                            Red_Den_mat[pos][orb][0][pos][orb][1] );
                    quant_s_z[0][pos][orb] = 0.5*real( Red_Den_mat[pos][orb][0][pos][orb][0] -
                            Red_Den_mat[pos][orb][1][pos][orb][1] );
                }
            }
        ...
        for(int pos=0;pos<Parameters_.ns;pos++){
            file_out<< quant_s_z[0][pos][0] + quant_s_z[0][pos][1] + quant_s_z[0][pos][2]<<"  ";
        }
        for(int pos=0;pos<Parameters_.ns;pos++){
            file_out<< quant_s_x[0][pos][0] + quant_s_x[0][pos][1] + quant_s_x[0][pos][2]<<"  ";
        }
        for(int pos=0;pos<Parameters_.ns;pos++){
            file_out<< quant_s_y[0][pos][0] + quant_s_y[0][pos][1] + quant_s_y[0][pos][2]<<"  ";
        }
            ...
            for(int wi=0;wi<n_wpoints;wi++){
                ...
                for(int pos_i=0;pos_i<Parameters_.ns;pos_i++){
                    for(int pos_j=0;pos_j<Parameters_.ns;pos_j++){
                        ...
                        /*               S_rw[pos_i][pos_j][wi] += one*cos((wi * dw) * (ts* dt_))*exp(-0.5*(ts*dt_*w_conv*ts*dt_*w_conv))*dt_*S_mag*S_mag*(  (cos(Theta_eq[pos_i])*cos(Theta[0][pos_j]))
                            +
                            (sin(Theta_eq[pos_i])*cos(Phi_eq[pos_i])*sin(Theta[0][pos_j])*cos(Phi[0][pos_j]) )
                            +
                            (sin(Theta_eq[pos_i])*sin(Phi_eq[pos_i])*sin(Theta[0][pos_j])*sin(Phi[0][pos_j]) )
                            );
*/
                        ...
                        T_rw[pos_i][pos_j][wi] +=0.0;

                        /*
                                exp(iota * (-wi * dw) * (ts* dt_))*exp(-0.5*(ts*dt_*w_conv*ts*dt_*w_conv))*dt_*  ( S_mag*(
                                                                                                                                        (
                                                                                                                                            (cos(Theta_eq[pos_i])*quant_s_z[0][pos_j])
                                                                                                                                        +
                                                                                                                                        (sin(Theta_eq[pos_i])*cos(Phi_eq[pos_i])*quant_s_x[0][pos_j])
                                                                                                                                    +
                                                                                                                                    (sin(Theta_eq[pos_i])*sin(Phi_eq[pos_i])*quant_s_y[0][pos_j])

                                )
                                +
                                (
                                    (quant_s_z_eq[pos_i]*cos(Theta[0][pos_j]))
                                +
                                (quant_s_x_eq[pos_i]*sin(Theta[0][pos_j])*cos(Phi[0][pos_j]) )
                                +
                                (quant_s_y_eq[pos_i]*sin(Theta[0][pos_j])*sin(Phi[0][pos_j]) )
                                ...
                                (
                                    (quant_s_x_eq[pos_i]*quant_s_x[0][pos_j])
                                +
                                (quant_s_y_eq[pos_i]*quant_s_y[0][pos_j])
                                +
                                (quant_s_z_eq[pos_i]*quant_s_z[0][pos_j])
                                ...
                                (
                                    S_mag*S_mag*(  (cos(Theta_eq[pos_i])*cos(Theta[0][pos_j]))
                                    +
                                    (sin(Theta_eq[pos_i])*cos(Phi_eq[pos_i])*sin(Theta[0][pos_j])*cos(Phi[0][pos_j]) )
                                +
                                (sin(Theta_eq[pos_i])*sin(Phi_eq[pos_i])*sin(Theta[0][pos_j])*sin(Phi[0][pos_j]) )
                        ...
                        */
                        ...
                        /*  S_rw[pos_i][pos_j][wi] += exp(iota * (wi * dw) * (ts* dt_))*dt_*(  cos(Theta[0][pos_i])*cos(Theta[0][pos_j])
                                +
                                sin(Theta[0][pos_i])*cos(Phi[0][pos_i])*sin(Theta[0][pos_j])*cos(Phi[0][pos_j])
                                +
                                sin(Theta[0][pos_i])*sin(Phi[0][pos_i])*sin(Theta[0][pos_j])*sin(Phi[0][pos_j])
                                );
...
#pragma omp parallel for default(shared) private(pi, ti, sx, sy, sz)
        ...
        for(int pos=0;pos<Parameters_.ns;pos++){
            ...
            for(int orb=0;orb<Parameters_.orbs;orb++){
                sx[orb]=0.5*real( Red_Den_mat_temp[pos][orb][1][pos][orb][0] + Red_Den_mat_temp[pos][orb][0][pos][orb][1] );
                sy[orb]=0.5*imag( Red_Den_mat_temp[pos][orb][1][pos][orb][0] - Red_Den_mat_temp[pos][orb][0][pos][orb][1] );
                sz[orb]=0.5*real( Red_Den_mat_temp[pos][orb][0][pos][orb][0] - Red_Den_mat_temp[pos][orb][1][pos][orb][1] );
            }

            Phi[ts+1][pos_x][pos_y] = Phi[ts][pos_x][pos_y];
            Theta[ts+1][pos_x][pos_y] = Theta[ts][pos_x][pos_y];

            for(int orb=0;orb<Parameters_.orbs;orb++){
                Phi[ts+1][pos_x][pos_y] +=  (dt_*(  -1.0*(Jval_array[pos]*((((sin(pi)*cos(ti))/(sin(ti)))*sy[orb]) + (((cos(pi)*cos(ti))/(sin(ti)))*sx[orb]) - sz[orb]))  -  Bval_array[pos]  )    );

                Theta[ts+1][pos_x][pos_y] +=  (dt_*( Jval_array[pos]*((cos(pi)*sy[orb]) - (sin(pi)*sx[orb]))));
            }

            //Phi[ts+1][pos] = Phi[ts][pos] ;
            //Theta[ts+1][pos] = Theta[ts][pos] ;

            //derivative_phi += (-1.0)*( Jval*((((sin(pi)*cos(ti))/(sin(ti)))*sy) + (((cos(pi)*cos(ti))/(sin(ti)))*sx) - sz));
            //derivative_theta += ( Jval*((cos(pi)*sy) - (sin(pi)*sx)));
            ...
            for(int ng=0;ng<basis.Neighbour[pos].size();ng++){
                ...
                Phi[ts+1][pos] += (-1.0)*(dt_*( (S_mag)*(Jval_p)* (((sin(tj)*cos(ti)*cos(pi - pj))/(sin(ti))) - cos(tj) )));
                //derivative_phi += (-1.0)*(( (S_mag)*(Jval_p)*(((sin(tj)*cos(ti)*cos(pi - pj))/(sin(ti))) - cos(tj) ) ));

                Theta[ts+1][pos] += (dt_*(S_mag)*(Jval_p)*(sin(tj)*sin(pj - pi)));
...
*/
            ...
            for (int ng=0;ng<8;ng++){
                ...
                Phi[ts+1][pos_x][pos_y] += (-1.0)*(dt_*( (S_mag)*(value_)* (((sin(tj)*cos(ti)*cos(pi - pj))/(sin(ti))) - cos(tj) )));
                Theta[ts+1][pos_x][pos_y] += (dt_*(S_mag)*(value_)*(sin(tj)*sin(pj - pi)));
...
#pragma omp parallel for default(shared) private(derivative_theta,derivative_phi, pi, ti, sx, sy, sz)
            ...
            for(int pos=0;pos<Parameters_.ns;pos++){
                ...
                derivative_phi = (-1.0)*( Jval_array[pos]*((((sin(pi)*cos(ti))/(sin(ti)))*sy) + (((cos(pi)*cos(ti))/(sin(ti)))*sx) -  sz ))  -  Bval_array[pos]; //-Bval_array[pos]
...
void SC_SW_ENGINE_VNE_3orbPnictides::Write_final_time_result(){
    ...
    for(int pi=0;pi<Red_Den_mat.size();pi++){
        for(int oi=0;oi<Red_Den_mat[pi].size();oi++){
            for(int si=0;si<Red_Den_mat[pi][oi].size();si++){

                for(int pj=0;pj<Red_Den_mat[pi][oi][si].size();pj++){
                    for(int oj=0;oj<Red_Den_mat[pi][oi][si][pj].size();oj++){
                        for(int sj=0;sj<Red_Den_mat[pi][oi][si][pj][oj].size();sj++){
                            file_out_DM<<pi<<"    "<<oi<<"   "<<si<<"    "<<pj<<"    "<<oj<<"   "<<sj<<"    "<<
                                         Red_Den_mat[pi][oi][si][pj][oj][sj].real()<<"  "<<
                                         Red_Den_mat[pi][oi][si][pj][oj][sj].imag()<<endl;
...
void SC_SW_ENGINE_VNE_3orbPnictides::Read_Restart_Data(){
            ...
            for(int si=0;si<2;si++){
                ...
                for(int pj=0;pj<Red_Den_mat.size();pj++){
                    for(int oj=0;oj<3;oj++){
                        for(int sj=0;sj<2;sj++){
                            file_in_DM>>temp>>temp>>temp>>temp>>temp>>temp>>temp_real>>temp_imag;
                            Red_Den_mat[pi][oi][si][pj][oj][sj]=(one*temp_real) + (iota*temp_imag);
...
void SC_SW_ENGINE_VNE_3orbPnictides::Read_parameters(string filename){
    ...
    cout<<"Jnn = "<<Parameters_.J_NN<<endl;
    cout<<"Jnnn = "<<Parameters_.J_NNN<<endl;
    cout<<"Jhund = "<<Parameters_.J_HUND<<endl;
